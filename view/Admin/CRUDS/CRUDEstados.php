        ...
        <div class="card-body">
            <form method="POST" id="estadoForm">
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    ...
                    <a href="?a=CRUDEstados" class="btn btn-secondary">Cancelar</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
        ...
        <div class="card-body">
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="?a=CRUDEstados&edit=<?= $estado['IdEstado'] ?>" 
                                    class="btn btn-sm btn-warning edit-btn" 
                                    data-id="<?= $estado['IdEstado'] ?>">
                                    <i class="bi bi-pencil"></i> Editar
                                </a>
                                <a href="?a=CRUDEstados&delete=<?= $estado['IdEstado'] ?>" 
            ...
            <nav aria-label="Page navigation">
                <ul class="pagination justify-content-center">
                    <?php if ($page > 1): ?>
                    <li class="page-item">
                        <a class="page-link" href="?a=CRUDEstados&page=<?= $page - 1 ?>" aria-label="Previous">
                    ...
                    <li class="page-item">
                        <a class="page-link" href="?a=CRUDEstados&page=<?= $i ?>"><?= $i ?></a>
                    </li>
                    <?php endfor; ?>
                    
                    <?php if ($page < $totalPages): ?>
                    <li class="page-item">
                        <a class="page-link" href="?a=CRUDEstados&page=<?= $page + 1 ?>" aria-label="Next">
...
document.addEventListener('DOMContentLoaded', function() {
    <?php if (isset($_GET['edit'])): ?>
    fetch('?a=getEstadoById&id=<?= $_GET['edit'] ?>')
    ...
    const cancelBtn = document.querySelector('a[href="?a=CRUDEstados"]');
    if (cancelBtn) {
        cancelBtn.addEventListener('click', function(e) {
            e.preventDefault();
            document.getElementById('estadoForm').reset();
            document.getElementById('IdEstado').value = '';
            window.location.href = '?a=CRUDEstados';
        });
    }
});
</script>
